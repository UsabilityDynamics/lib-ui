/**
 * Utilized for API Documentation and Explorer on UD API services and on client API endpoints.
 *
 * ud.documentation.js v0.2.0
 * http://usabilitydynamics.com
 *
 * @version 0.2.0
 * @description Utilized for API Documentation and Explorer on UD API services and on client API endpoints
 * @package UD
...
define( "ui.markdown", function() {
  ...
  jQuery( function() {
    ...
    if( !ud || !ud.load ) {
      return console.error( 'UD Load not available.' );
    }

    // Load CSS for API Documentation
    ud.load.css( '//ud-cdn.com/js/ud.documentation/0.1/assets/documentation-styles.css' );

    ud.load.js( {
      'makeHtml': '//ud-cdn.com/js/showdown/latest/showdown.js',
      'jQuery.prototype.slideto': '//api.usabilitydynamics.com/documentation/js/jquery.slideto.min.js',
      'jQuery.prototype.wiggle': '//api.usabilitydynamics.com/documentation/js/jquery.wiggle.min.js',
      'jQuery.bbq': '//api.usabilitydynamics.com/documentation/js/jquery.ba-bbq.min.js',
      'Handlebars': '//api.usabilitydynamics.com/documentation/js/handlebars.runtime-1.0.0.beta.6.js',
      '_': '//api.usabilitydynamics.com/documentation/js/underscore-min.js',
      'backbone': '//api.usabilitydynamics.com/documentation/js/backbone-min.js',
      'async': '//ud-cdn.com/js/async/1.0/async.js',
      'swaggerUi': '//api.usabilitydynamics.com/documentation/js/swagger-ui.js' }, function() {

      async.auto( {

        // Load App Configuration
        'system': function( _next, _report ) { // console.info( 'system()' );

          //jQuery( true, app, _report );

          jQuery.get( '/system.json',function( data ) {

            return _next( null, jQuery.extend( true, app, ( data && data.system ? data.system : {} ) ) );
        ...
        // Check if Authenticate is necessary
        'authenticate': [ 'system', function( _next, _report ) { // console.info( 'authenticate()' );

          if( _report.system.name ) {
            jQuery( '.product em.product-name' ).html( _report.system.name );
          }

          if( _report.system.version ) {
            jQuery( '.product em.version' ).html( _report.system.version );
          }

          if( !_report.system.must_authenticate ) { // console.info( 'authenticate().success() - Skipped.' );
        ...
        // Load Readme
        'readme': [ 'authenticate', function( _next, _report ) { // console.info( 'readme()' );

          jQuery.get( '/readme.md',function( data ) {

            // If makeHTML was loaded as a client-side script, use it
            if( typeof makeHtml === 'function' && data ) {
              _report.readme = makeHtml( data );
            }

            jQuery( '#documentation' ).html( _report.readme );
        ...
        // API Explorer
        'explorer': [ 'readme', function( _next, _report ) { // console.info( 'explorer()' );

          app.SwaggerApi = new SwaggerApi( {
            'discoveryUrl': window.location.origin + ( app && app.url ? app.url.replace( 'system.json', 'resources.json' ) : ''),
            'api_key': '',
            'progress': function() {
            }
          } );
  ...
  (function() {
    var SwaggerApi, SwaggerModel, SwaggerModelProperty, SwaggerOperation, SwaggerRequest, SwaggerResource, __bind = function( fn, me ) {
      return function() {
    ...
    SwaggerApi = (function() {
      ...
      SwaggerApi.prototype.discoveryUrl = "http://cloud.usabilitydynamics.com/resources.json";
    ...
    SwaggerOperation = (function() {
      ...
      function SwaggerOperation( nickname, path, httpMethod, parameters, summary, notes, responseClass, errorResponses, resource, supportedContentTypes ) {
        var parameter, v, _i, _j, _len, _len1, _ref, _ref1, _ref2, _this = this;
